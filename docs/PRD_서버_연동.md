...
**결정: MVP는 평문 저장, 앱 단 암호화는 백로그로 보류.**

- `GCP` Persistent Disk는 기본적으로 디스크 자체가 암호화됨 — "서버가 물리적으로 도난당해 디스크를 뜯어보는" 시나리오는 이미 무료로 막혀있음
- 남는 위험은 "서버에 원격으로 침입해 실행 중인 프로그램 입장에서 파일을 읽어가는" 경우뿐인데, 앱 단 암호화(`SQLCipher` 등)를 해도 복호화 키가 결국 같은 서버 어딘가에 있어야 해서 침입 시 키까지 같이 노출될 가능성이 큼 — 키를 `GCP Secret Manager` 같은 외부 금고로 분리해야 실효성이 생기는데 구현 부담이 큼
- §4.1에서 서버가 들고 있는 정보 자체를 이미 최소화함(상세 복용 이력 없음) — 노출돼도 피해 범위가 작음
- **백로그**: 사용자가 늘거나 서버가 더 민감한 정보를 갖게 되면, `SQLCipher` + `Secret Manager` 조합으로 업그레이드 검토
...
**결정: MVP는 동기화 안 함(그냥 계속 발송 시도), 백로그로 개선 예정.**

- 사용자가 알림을 끌 수 있는 곳은 두 군데 — 앱 안 토글(자체 설정)과 iOS 시스템 알림 권한(앱이 스스로는 못 봄)
- MVP: 둘 다 서버에 동기화 안 함 — 꺼져 있어도 서버는 계속 발송 시도(iOS가 시스템 권한 꺼진 건 알아서 걸러줌, 앱 내 토글만 꺼진 경우는 나중 개선 대상)
- **백로그(추후 추가)**:
  - 앱 내 토글: 끄는 즉시 기존 엔드포인트(`PATCH /pills/{pill_id}/cycle` 등)에 상태 필드 추가해서 반영
  - 시스템 권한: `heartbeat` 호출에 현재 권한 상태를 같이 실어 보내 앱 열릴 때마다 동기화
  - 새 엔드포인트 없이 기존 인프라에 얹는 방향으로 설계해둠 (MVP 이후 착수)
...
주요 설계 결정은 모두 완료됨. 아래는 결정 완료 항목 요약(상세는 각 섹션 참고):

| 항목 | 결정 | 근거 |
|---|---|---|
| 이탈 감지 알림 중단 기준 | 14일 (잠정, env로 조정 가능) | §6, §9.1, §4.3 |
| 알림 비활성화 동기화 | MVP는 미동기화, 추후 토글 즉시반영 + heartbeat 동기화 추가 | §6.3 |
| 건강 데이터 암호화 | MVP는 평문(GCP 기본 디스크 암호화 활용), 추후 SQLCipher+Secret Manager 검토 | §4.6 |
| GCP 프로젝트 구성 | DoSurf-API와 별도 프로젝트 | §12.6 |
| SQLite → Postgres 전환 | DAU 숫자 아닌 `SQLITE_BUSY` 에러 빈도로 실측 판단 | §14.1 |

실제 남은 할 일(설계 결정이 아닌 작업)은 §11 마일스톤 참고.

## 11. 마일스톤 제안

0. GCP 프로젝트 세팅 (§12 결정: e2-micro) 후 서버 배포 환경 구성
1. API 계약 재정의 및 문서화 (§7 리스크 반영, 응답 스키마 고정, `PILLING_SERVER_API_GUIDE.md`에 heartbeat/cycle/message 엔드포인트 반영)
...
**결정: `DoSurf-API`와 별도 프로젝트로 분리.**

- `DoSurf-API`는 Firebase(서버리스) 기반이라 애초에 별개 프로젝트로 존재 — 자연스럽게 이미 분리돼 있는 상태
- 같은 `빌링 계정`(요금 단위) 아래 프로젝트를 나눠도 e2-micro 무료 한도엔 영향 없음(§12.2에서 확인) — 분리해도 비용 손해 없음
- 프로젝트를 나누면 `IAM` 권한 경계도 분리됨 — Pilling 쪽 키가 유출돼도 DoSurf-API 리소스는 안전, 로그/모니터링도 안 섞임
- 이 문서 전반에서 이어온 "건강정보는 위험 범위를 최대한 좁게"라는 원칙(§4.1, §5.1, §4.5)과 일관됨
- `Cloud Monitoring` 웹훅 + `send_telegram_alert()` 설정은 프로젝트별로 한 번씩 세팅 필요(일회성 비용) — §13 참고
...
### 14.1 Postgres 전환 트리거

**결정: DAU 숫자를 미리 정해두지 않고, 실측 신호(`SQLITE_BUSY` 에러 빈도)로 판단.**

- `SQLite`는 동시 쓰기 요청이 오면 순서대로 줄 세워 처리(데이터 유실 없음) — 대기가 너무 길어지면 `SQLITE_BUSY` 에러가 남
- DAU 기준 숫자는 추측일 뿐이라(§12.1과 같은 함정) 신뢰하지 않음. 대신 `SQLITE_BUSY` 에러가 로그에 일정 빈도 이상 찍히기 시작하는 순간을 실제 병목 신호로 삼음
- 구현: §13 `send_telegram_alert()` 인프라에 "`SQLITE_BUSY` 하루 N회 이상 → WARNING 알림" 하나만 추가 — 새 인프라 불필요
- 전제: 애플리케이션 레벨에서 `busy_timeout` 설정 + 에러를 조용히 삼키지 않고 로깅(§7 "무음 실패 금지" 원칙과 동일)
